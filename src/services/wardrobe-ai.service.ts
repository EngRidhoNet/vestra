 ...
 */
export async function extractWardrobeItem(
  imageBase64: string,
  sendChatFn: (prompt: string, imageBase64: string) => Promise<string>
) {
  const prompt = `
...
Do not include markdown code blocks, just raw JSON.`;
  ...
  const rawResponse = await sendChatFn(prompt, imageBase64);
...
export async function getRecommendedOutfit() {}
